...
public:
    ...
    void change_position()  // it is executed when the bubble strikes the shooter .
    {
        if(body.getX()>250){  // if the x coordinate of shooter > 250 then shooter moves to midpoint of the first half .
             body.moveTo(125,450-body.getHeight()/2);
             head.moveTo(125,450-body.getHeight()-head.getRadius());
             return ;
        }
        else {    // if the x coordinate of shooter < 250 then shooter moves to midpoint of the second half .
             body.moveTo(375,450-body.getHeight()/2);
             head.moveTo(375,450-body.getHeight()-head.getRadius());
             return ;
        }
